...
export class PlikSidebarView extends ItemView {
	...
	render(): void {
		...
		btn.addEventListener('click', () => this.openTargetPicker());

		contentEl.createEl('hr');

		const header = contentEl.createDiv({ cls: 'setting-item' });
		header.createEl('div', { text: t('sidebar.history'), cls: 'setting-item-name' });
		...
		if (history.length === 0) {
			contentEl.createEl('div', { text: t('sidebar.empty'), cls: 'setting-item-description' });
			return;
		}
